...
import { Loader2, Info, Languages, Type, AlignLeft, Calendar, Tag } from "lucide-react"
...
export default function AboutDetailsPage() {
  ...
  const { data: response, isLoading, isError } = useAbout(id)
  const t = useTranslations("About")

  if (isLoading) {
    return (
      <PermissionGuard permission="show-about">
  ...
  // The API might return { data: AboutItem } or a paginated array { data: AboutItem[] }
  const rawData = response?.data
  const about = Array.isArray(rawData) ? rawData[0] : rawData
  ...
  const translations = [
    { lang: "ar", label: t("ar"), data: about.ar },
    { lang: "en", label: t("en"), data: about.en },
    { lang: "tr", label: t("tr"), data: about.tr },
    { lang: "fa", label: t("fa"), data: about.fa },
    { lang: "ms", label: t("ms"), data: about.ms },
    { lang: "iid", label: t("iid"), data: about.iid },
  ]
        ...
        <Card className="rounded-2xl border-zinc-100 dark:border-zinc-800 shadow-sm overflow-hidden bg-white dark:bg-zinc-950">
          <div className="flex flex-col md:flex-row md:items-start justify-between p-6 gap-6">
            <div className="flex items-center gap-4">
              ...
              <div>
                <p className="text-sm text-zinc-500 font-bold uppercase tracking-wider mb-1">{t("about_details")}</p>
                <h1 className="text-2xl font-black text-zinc-900 dark:text-zinc-100 leading-tight">
                  {about.ar?.title || about.en?.title || "-"}
                </h1>
              ...
              <Card key={item.lang} className="rounded-2xl border-zinc-100 dark:border-zinc-800 shadow-sm overflow-hidden h-full flex flex-col">
                  ...
                  <CardContent className="p-5 flex-1 flex flex-col gap-4">
                      ...
                      <div>
                          ...
                          <div className="text-sm font-bold text-zinc-900 dark:text-zinc-100 bg-zinc-50 dark:bg-zinc-900/50 p-3 rounded-xl border border-zinc-100 dark:border-zinc-800 min-h-[44px]">
                              {item.data?.slug || <span className="text-zinc-400 font-normal">-</span>}
                          </div>
                      </div>
