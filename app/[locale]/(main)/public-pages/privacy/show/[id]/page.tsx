...
import { usePolicy } from "@/features/policies/hooks"
...
export default function PrivacyDetailsPage() {
  ...
  const { data: response, isLoading, isError } = usePolicy(id)
  ...
  const translations = [
    { lang: "ar", label: t("ar"), data: policy.ar },
    { lang: "en", label: t("en"), data: policy.en },
    { lang: "tr", label: t("tr"), data: policy.tr },
    { lang: "fa", label: t("fa"), data: policy.fa },
    { lang: "ms", label: t("ms"), data: policy.ms },
    { lang: "iid", label: t("iid"), data: policy.iid },
  ]
      ...
      <Card className="rounded-2xl border-zinc-100 dark:border-zinc-800 shadow-sm overflow-hidden bg-white dark:bg-zinc-950">
        <div className="flex flex-col md:flex-row md:items-start justify-between p-6 gap-6">
          <div className="flex items-center gap-4">
            ...
            <div>
              <p className="text-sm text-zinc-500 font-bold uppercase tracking-wider mb-1">{t("privacy_details")}</p>
              <h1 className="text-2xl font-black text-zinc-900 dark:text-zinc-100 leading-tight">
                {policy.ar?.title || policy.en?.title || "-"}
              </h1>
