...
import { useBlogItem } from "@/features/blogs/hooks"
...
export default function BlogDetailsPage() {
  ...
  const { data: response, isLoading, isError } = useBlogItem(id)
  const t = useTranslations("Blogs")

  if (isLoading) {
    return (
      <PermissionGuard permission="show-blog">
        ...
        <Card className="rounded-2xl border-zinc-100 dark:border-zinc-800 shadow-sm overflow-hidden">
          ...
          {blogItem.images?.image ? (
              <div className="w-full h-64 md:h-80 relative overflow-hidden bg-zinc-100 dark:bg-zinc-900 border-b border-zinc-100 dark:border-zinc-800">
                  <Image width={400} height={400} src={blogItem.images.image} 
                      alt={blogItem.alt || blogItem.title} 
          ...
          <div className="flex items-center justify-between bg-zinc-50/50 dark:bg-zinc-900/50 p-6 border-b border-zinc-100 dark:border-zinc-800">
            <div>
              <div className="flex items-center gap-3 mb-2">
                  <Badge variant={blogItem.is_active ? "default" : "secondary"} className="rounded-lg text-sm px-3 py-1">
                      {blogItem.is_active ? t("active") : t("inactive")}
                  </Badge>
