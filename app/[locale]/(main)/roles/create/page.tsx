...
export default function CreateRolePage() {
  ...
  const { register, handleSubmit, setValue, watch, formState: { errors } } = useForm<UpdateRoleValues>({
  ...
  if (isPermsError || !allPermissions) {
    return (
      <div className="flex h-[300px] w-full items-center justify-center">
        <p className="text-lg font-medium text-zinc-500">{t("not_found", { fallback: "البيانات غير موجودة" })}</p>
      </div>
    )
  }
  ...
  return (
    <form onSubmit={handleSubmit(onSubmit)} className="space-y-6 pb-10 max-w-5xl mx-auto mt-6">
      <div className="flex justify-between items-center">
        <h1 className="text-2xl font-black text-zinc-900 dark:text-zinc-100">{t("create_role", { fallback: "إضافة دور" })}</h1>
        <Button type="submit" disabled={createMutation.isPending} className="gap-2">
          {createMutation.isPending ? <Loader2 className="h-4 w-4 animate-spin" /> : <Save className="h-4 w-4" />}
          {t("create_role", { fallback: "إضافة دور" })}
        </Button>
      </div>
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
        ...
        {Object.keys(allPermissions).length === 0 && (
          <div className="col-span-full flex flex-col items-center justify-center h-48 bg-white dark:bg-zinc-950 rounded-2xl border border-zinc-100 dark:border-zinc-800 text-zinc-400">
            ...
            <p className="font-medium text-lg">{t("no_permissions", { fallback: "لا توجد صلاحيات" })}</p>
          </div>
        )}
      </div>
    </form>
