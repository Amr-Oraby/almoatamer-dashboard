...
import { useUpdateLanguage, useLanguage } from "../hooks";
import { updateLanguageSchema, UpdateLanguageFormValues } from "../schemas";
...
export function UpdateLanguageForm({ languageId }: UpdateLanguageFormProps) {
    const t = useTranslations("Dashboard");
    const router = useRouter();
    const localeCode = useLocale();
    const [currentFlag, setCurrentFlag] = useState<string | null>(null);

    const { data: languageData, isLoading: isLoadingLanguage } = useLanguage(languageId);
    const { mutateAsync: updateLanguage, isPending } = useUpdateLanguage(languageId);

    const { register, handleSubmit, reset, formState: { errors } } = useForm<UpdateLanguageFormValues>({
    ...
    const onSubmit = async (data: UpdateLanguageFormValues) => {
        try {
    ...
    return (
        <form onSubmit={handleSubmit((data) => onSubmit(data as UpdateLanguageFormValues))} className="w-full space-y-6">
            ...
            <div className="bg-white dark:bg-zinc-950 border border-zinc-200 dark:border-zinc-800 rounded-2xl p-5 md:p-6 space-y-6 shadow-sm">
                ...
                <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                    <div className="space-y-2">
                        ...
                        {currentFlag && (
                            <div className="mb-2">
                                <Image src={currentFlag} alt="Current Flag" width={40} height={40} className="rounded-md object-cover border border-zinc-200 dark:border-zinc-700" unoptimized />
                            </div>
                        )}
                        <input
                            type="file"
