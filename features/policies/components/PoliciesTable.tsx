...
import { PolicyItem } from "@/features/policies/types"
...
export function PoliciesTable() {
  ...
  const columns = useMemo<ColumnDef<PolicyItem>[]>(() => [
    {
      id: "index",
      header: "#",
      size: 60,
      ...
      cell: ({ row }) => {
        return (
          <div className="flex items-center justify-center">
            <TableActionMenu items={[{ text: tCommon("details", { fallback: "Details" }), href: `/public-pages/privacy/show/${row.original.id}`, permission: "show-policy" }]} />
          </div>
        )
      },
    },
