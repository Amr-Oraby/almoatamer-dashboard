...
import { useMoatmr, useUpdateMoatmr } from "../hooks";
import { Loader2 } from "lucide-react";
import { updateMoatmrSchema, UpdateMoatmrValues } from "../schemas";
...
interface UpdateMoatmrFormProps {
    moatmrId: string;
}

export function UpdateMoatmrForm({ moatmrId }: UpdateMoatmrFormProps) {
    const t = useTranslations("Almoatamers");
    const tClients = useTranslations("Clients");
    const router = useRouter();
    
    const { data: response, isLoading: isLoadingMoatmr } = useMoatmr(moatmrId);
    const { mutateAsync: updateMoatmr, isPending } = useUpdateMoatmr(moatmrId);
    ...
    const { register, handleSubmit, reset, setValue, watch, formState: { errors } } = useForm<UpdateMoatmrValues>({
        resolver: zodResolver(updateMoatmrSchema),
        defaultValues: {
            is_active: false,
            is_available: false,
        }
    ...
    useEffect(() => {
        if (response?.data) {
            ...
            reset({
                ...
                language_id: c.locale ? languages.find((l: any) => l.short_name === c.locale)?.id || "" : "",
                latitude: c.latitude || "",
                longitude: c.longitude || "",
                from_price: c.from_price_umrah || "",
                to_price: c.to_price_umrah || "",
                is_active: c.is_active,
                is_available: c.is_available === 1,
            });
            if (c.image) {
    ...
    async function onSubmit(values: UpdateMoatmrValues) {
    ...
    if (isLoadingMoatmr || isLoadingCountries || isLoadingLanguages) {
        return (
    ...
    return (
        <FormLayout 
            title={t("update", { fallback: "Update Moatamer" })}
            description={t("update_desc", { fallback: "Modify moatamer information" })}
        >
