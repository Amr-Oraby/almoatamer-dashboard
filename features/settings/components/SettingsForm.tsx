...
export function SettingsForm() {
  ...
  async function onUpdateSubmit(values: Record<string, string | number>) {
    if (!settingsResponse?.data) return;
    try {
      const promises = settingsResponse.data.map(async (setting) => {
  ...
  async function onCreateSubmit(values: { key: string; value: string }) {
  ...
  const displaySettings = settingsResponse?.data
    ? settingsResponse.data.filter(setting => setting.key !== "font_colors" && setting.key !== "discount_cards")
    : [];
    ...
    <div className="flex flex-col gap-8 w-full max-w-[1400px] mx-auto">
      ...
      <div className="w-full bg-white dark:bg-zinc-950 rounded-2xl border border-zinc-200 dark:border-zinc-800 shadow-sm overflow-hidden transition-all hover:shadow-md">
        ...
        <div className="p-6 md:p-8">
          <form onSubmit={handleCreateSubmit(onCreateSubmit)} className="flex flex-col md:flex-row gap-6 items-end">
            <div className="flex-1 w-full space-y-3">
              ...
              <input
                ...
                {...registerCreate("key", { required: true })}
              />
            </div>
            ...
            <div className="flex-1 w-full space-y-3">
              ...
              <input
                ...
                className="w-full h-12 bg-zinc-50 dark:bg-zinc-900 border border-zinc-200 dark:border-zinc-800 rounded-xl px-4 text-base focus:outline-none focus:ring-2 focus:ring-primary/20 focus:border-primary transition-all placeholder:text-zinc-400"
                {...registerCreate("value", { required: true })}
              />
            </div>
            
            <Button type="submit" disabled={isCreating} className="w-full md:w-auto h-12 px-8 rounded-xl font-medium shadow-sm transition-all hover:shadow text-base">
