...
import { useMainSectionInfo, useUpdateMainSectionInfo } from "../hooks";
...
export function MainSectionForm() {
  ...
  const { data: response, isLoading } = useMainSectionInfo();
  const { mutateAsync: updateMainSection, isPending } = useUpdateMainSectionInfo();
  
  const [previewImage, setPreviewImage] = useState<string | null>(null);
  const [imageFile, setImageFile] = useState<File | null>(null);

  const { register, handleSubmit, reset } = useForm<MainSectionFormValues>({
  ...
  async function onSubmit(values: MainSectionFormValues) {
    const formData = new FormData();
    const type = response?.data?.type || "umrah_section";
    ...
    try {
      await updateMainSection(formData);
    } catch (error) {
      console.error(error);
    }
  }
  ...
  return (
    <FormLayout 
      title={t("main_section", { fallback: "Main Section Info" })}
      description={t("main_section_desc", { fallback: "Manage main section information." })}
    >
